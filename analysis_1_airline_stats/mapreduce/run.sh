...
if [ "${CLUSTER_MODE:-false}" = "true" ]; then
    ...
    MR_OUTPUT="s3://${S3_BUCKET}/${S3_PREFIX}/results/analysis_1/mapreduce/$(date +%s)"
...
echo ""
echo "=== Prime 10 righe ==="
if [ "${CLUSTER_MODE:-false}" != "true" ] && [ -f "results/analysis_1/mapreduce/output.csv" ]; then
    head -n 10 "results/analysis_1/mapreduce/output.csv"
fi
